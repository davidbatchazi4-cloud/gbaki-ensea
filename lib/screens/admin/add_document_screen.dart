// lib/screens/admin/add_document_screen.dart — v3
...
class _AddDocumentScreenState extends State<AddDocumentScreen> {
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                ...
                items: widget.ues.isEmpty
                    ? [const DropdownMenuItem(value: null, child: Text('Aucune UE — créez-en d\'abord'))]
                    : widget.ues.map((ue) => DropdownMenuItem(
                          value: ue.id,
                          child: Text('${ue.code} — ${ue.nom}', style: const TextStyle(fontSize: 13)),
                        )).toList(),
                onChanged: widget.ues.isEmpty
                    ? null
                    : (val) => setState(() {
                          _selectedUEId = val;
              ...
              DropdownButtonFormField<String>(
                ...
                items: _matieres.isEmpty
                    ? [const DropdownMenuItem(value: null, child: Text('Sélectionnez une UE d\'abord'))]
                    : _matieres.map((m) => DropdownMenuItem(
                          value: m.id,
                          child: Text(m.nom, style: const TextStyle(fontSize: 13)),
                        )).toList(),
                onChanged: _matieres.isEmpty
                    ? null
                    : (val) => setState(() => _selectedMatiereId = val),
              ...
              TextFormField(
                ...
                validator: (v) => v == null || v.isEmpty ? 'L\'URL est requise' : null,
              ),
  ...
  void _addDocument() {
    ...
    context.read<AppProvider>().addDocument(document);
      ...
      SnackBar(
        content: Row(
          children: [
            ...
            const SizedBox(width: 8),
            Expanded(child: Text('"${document.titre}" ajouté avec succès')),
          ],
        ),
